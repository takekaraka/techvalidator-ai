...
async function tryReelsDownloaderAPI(url) {
  ...
  try {
    ...
    if (data1?.video_url || data1?.url || data1?.download_url) {
      return data1.video_url || data1.url || data1.download_url;
    }
  } catch (e) {
    console.log('  → easeapi falló:', e.message);
  }

  // API 2: 7sCORP (fallback)
  try {
    const options2 = {
      method: 'GET',
    ...
    if (data2?.success && data2?.data?.content?.media_url) {
      return data2.data.content.media_url;
    }
  } catch (e) {
    console.log('  → 7sCORP falló:', e.message);
  }

  // API 3: instagram120 (segundo fallback)
  try {
    const options3 = {
      method: 'GET',
      hostname: 'instagram120.p.rapidapi.com',
      path: `/api/instagram/links?url=${encodeURIComponent(url)}`,
      headers: {
    ...
    if (data3?.video_url || data3?.url) {
      return data3.video_url || data3.url;
    }
  } catch (e) {
    console.log('  → instagram120 falló:', e.message);
  }
