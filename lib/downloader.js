...
async function tryIGram(url) {
  ...
  throw new Error('No video from iGram');
}

async function trySnapInsta(url) {
  const options = {
    method: 'GET',
    hostname: 'snapinsta.io',
    path: `/api/download?url=${encodeURIComponent(url)}`,
    headers: {
      'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
    },
  };
  ...
  if (videoUrl) return videoUrl;
  throw new Error('No video from SnapInsta');
}
...
async function fetchInstagramVideoURL(url) {
  ...
  if (apiKey) {
    try {
      console.log('  → Intentando RapidAPI (smvd)...');
      return await tryRapidAPI1(url, apiKey);
    } catch (e) {
      errors.push(`RapidAPI1: ${e.message}`);
    }

    // Strategy 2: RapidAPI Secondary
    try {
      console.log('  → Intentando RapidAPI (scraper-api2)...');
      return await tryRapidAPI2(url, apiKey);
    } catch (e) {
      errors.push(`RapidAPI2: ${e.message}`);
    }

    // Strategy 3: RapidAPI Tertiary
    try {
      console.log('  → Intentando RapidAPI (looter2)...');
      return await tryRapidAPI3(url, apiKey);
    } catch (e) {
      errors.push(`RapidAPI3: ${e.message}`);
    }
  }

  // Strategy 4: Free iGram
  try {
    console.log('  → Intentando iGram (gratis)...');
    return await tryIGram(url);
  } catch (e) {
    errors.push(`iGram: ${e.message}`);
  }

  // Strategy 5: Free SnapInsta
  try {
    console.log('  → Intentando SnapInsta (gratis)...');
    return await trySnapInsta(url);
  } catch (e) {
    errors.push(`SnapInsta: ${e.message}`);
  }

  throw new Error(`Instagram: APIs no disponibles. Descarga el video y súbelo directamente.`);
}
