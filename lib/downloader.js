...
const YT_DLP_PATHS = ['/usr/local/bin/yt-dlp', '/usr/bin/yt-dlp', 'yt-dlp'];
...
async function tryReelsDownloaderAPI(url) {
  ...
  // API 1: codecrest8 instagram-reels-downloader2 (MÁS CONFIABLE)
  try {
    console.log('  → Probando codecrest8...');
    const options1 = {
      method: 'GET',
      hostname: 'instagram-reels-downloader2.p.rapidapi.com',
      path: `/reels?link=${encodeURIComponent(url)}`,
      headers: {
        'x-rapidapi-host': 'instagram-reels-downloader2.p.rapidapi.com',
        'x-rapidapi-key': apiKey,
      },
    };
    const { data: data1 } = await httpsRequest(options1);
    const videoUrl = data1?.video_url || data1?.url || data1?.data?.video_url || data1?.media;
    if (videoUrl) {
      console.log('  ✓ codecrest8 funcionó');
      return videoUrl;
    }
  } catch (e) {
    console.log('  → codecrest8 falló:', e.message);
  }

  // API 2: easeapi (fallback)
  try {
    console.log('  → Probando easeapi...');
    const options2 = {
      method: 'GET',
      hostname: 'instagram-reels-downloader-api.p.rapidapi.com',
      path: `/download?url=${encodeURIComponent(url)}`,
      headers: {
        'x-rapidapi-host': 'instagram-reels-downloader-api.p.rapidapi.com',
        'x-rapidapi-key': apiKey,
      },
    };
    const { data: data2 } = await httpsRequest(options2);
    if (data2?.video_url || data2?.url || data2?.download_url) {
      return data2.video_url || data2.url || data2.download_url;
    }
  } catch (e) {
    console.log('  → easeapi falló:', e.message);
  }

  // API 3: 7sCORP (fallback)
  try {
    console.log('  → Probando 7sCORP...');
    const options3 = {
      method: 'GET',
    ...
    const { data: data3 } = await httpsRequest(options3);
    if (data3?.success && data3?.data?.content?.media_url) {
      return data3.data.content.media_url;
    }
  } catch (e) {
    console.log('  → 7sCORP falló:', e.message);
  }

  throw new Error('Todas las APIs de Instagram fallaron. Intenta subir el video directamente.');
}
...
export async function downloadFromURL(url, filename) {
  ...
  const ytdlpAvailable = ytdlp !== null;

  // ─── Instagram ───
  if (isInstagramURL(url)) {
    console.log(`📱 Descargando desde Instagram...`);

    // API primero (más confiable en servidores)
    try {
      console.log(`  → Intentando API de descarga...`);
      const videoUrl = await fetchInstagramVideoURL(url);
      console.log(`  ✓ URL obtenida, descargando MP4...`);
      await downloadFile(videoUrl, outputPath);
      console.log(`  ✓ Descarga completada: ${filename}`);
      return outputPath;
    } catch (apiError) {
      console.warn(`  ⚠️ API falló: ${apiError.message}`);
    }

    // Fallback a yt-dlp (funciona mejor en local)
    if (ytdlpAvailable) {
      try {
        console.log(`  → Intentando yt-dlp...`);
