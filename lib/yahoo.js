...
export async function fetchYahooByUids(uids, { mailbox = 'INBOX' } = {}) {
  ...
  try {
    // Batched fetch sin parse: solo raw .eml + envelope.
    // Saltarse simpleParser ahorra ~2x memoria (no parsea HTML ni decodifica
    // attachments base64 en memoria). El .eml crudo ya tiene todo el contenido
    // y se sube tal cual a Drive.
    const uidList = uids.map((u) => String(u)).join(',');
    for await (const msg of client.fetch(
      ...
      try {
        ...
        const fromEnv = (env.from && env.from[0]) || {};
        messages.push({
          uid: String(msg.uid),
          from: {
            name: fromEnv.name || '',
            address: fromEnv.address || (fromEnv.mailbox && fromEnv.host ? `${fromEnv.mailbox}@${fromEnv.host}` : ''),
          },
          ...
          html: '',
          attachments: [],
          raw: msg.source, // Buffer crudo, sin parsear
        });
      } catch (e) {
        console.error(`fetchYahooByUids: uid=${msg.uid} failed:`, e.message);
      }
    }
  } catch (e) {
    console.error('fetchYahooByUids: fetch failed:', e.message);
  } finally {
    lock.release();
    await client.logout();
  }
