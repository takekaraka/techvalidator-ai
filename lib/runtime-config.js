...
const KEYS = [
  ...
  'GOOGLE_REDIRECT_URI',
  'DRIVE_ROOT_FOLDER',
  // Metadata cacheada (JSON-stringified): info de la carpeta raíz de Drive
  // ({id, url, name, saved_at}). Persistida para sobrevivir reinicios y poder
  // recuperar la URL aún con el server caído (cat data/runtime-config.json).
  'DRIVE_ROOT_FOLDER_INFO',
];
...
// Backup completo (incluye tokens OAuth si existen) — para portar a otra Mac/Render.
export function backupConfig() {
  const cfg = read();
  // Incluir tokens de Drive si existen en disco.
  const tokenFile = path.join(path.dirname(FILE), 'google-token.json');
  let driveTokens = null;
  if (fs.existsSync(tokenFile)) {
    try { driveTokens = JSON.parse(fs.readFileSync(tokenFile, 'utf8')); } catch (_) {}
  }
  return {
    version: 1,
...
export function restoreConfig(backup) {
  if (!backup || backup.version !== 1) {
    throw new Error('Backup no válido (version != 1)');
  }
  if (backup.config) {
    setRuntimeConfig(backup.config);
  }
  if (backup.drive_tokens) {
    const tokenFile = path.join(path.dirname(FILE), 'google-token.json');
    fs.writeFileSync(tokenFile, JSON.stringify(backup.drive_tokens, null, 2));
  }
  return { restored_keys: Object.keys(backup.config || {}), drive: Boolean(backup.drive_tokens) };
}
