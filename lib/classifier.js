...
// Circuit breaker: si Gemini devuelve 429 (quota), no llamamos más en este batch
// para no quemar tiempo en retries del SDK que tardan ~15s cada uno.
let quotaTrippedUntil = 0;
function isQuotaTripped() {
  return Date.now() < quotaTrippedUntil;
}
...
Responde SOLO con el JSON.`;
  ...
  try {
    ...
    if (e.message?.includes('429') || e.message?.includes('quota')) {
      // Quota agotada: paramos llamadas durante 5 min.
      quotaTrippedUntil = Date.now() + 5 * 60 * 1000;
      console.warn('[classifier] quota tripped, degrading to heuristic for 5 min');
    } else {
