...
export async function uploadClassifiedEmails(items, { rootName } = {}) {
  ...
  for (const item of items) {
    ...
    // Solo el .eml: ya contiene los attachments embebidos en base64.
    // Subir attachments separados consume memoria 2x y mata Render free.
    const emlBuf = Buffer.isBuffer(rawEml) ? rawEml : Buffer.from(rawEml || '', 'utf8');
    // Salto emails > 20MB: matan memoria y rara vez son útiles
    if (emlBuf.length > 20 * 1024 * 1024) {
      console.warn(`drive: skipping ${emlName} (${(emlBuf.length / 1024 / 1024).toFixed(1)}MB)`);
      uploaded.push({ uid: email.uid, topic: topicName, skipped: 'too_large', size: emlBuf.length });
      continue;
    }
